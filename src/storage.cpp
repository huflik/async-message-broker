...
void Storage::CreateTables() {
    ...
    if (rc != SQLITE_OK) {
        std::string error = errmsg ? errmsg : "unknown error";
        sqlite3_free(errmsg);
        spdlog::warn("Failed to create some indexes: {}", error);
    }
...
uint64_t Storage::SaveMessageWithStatus(const Message& msg, int status) {
    ...
    sqlite3_bind_int64(stmt, 3, static_cast<sqlite3_int64>(msg.GetCorrelationId()));
    ...
    if (rc != SQLITE_DONE) {
        ...
        spdlog::error("Failed to insert message: {}", sqlite3_errmsg(db_));
        return 0;
    }
    
    uint64_t message_id = static_cast<uint64_t>(sqlite3_last_insert_rowid(db_));
    sqlite3_finalize(stmt);
    
    spdlog::debug("Message saved to database, id: {}, status: {}, corr_id: {}", 
                  message_id, status, msg.GetCorrelationId());
    return message_id;
}
...
void Storage::MarkDelivered(uint64_t message_id) {
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("Failed to prepare update statement: {}", sqlite3_errmsg(db_));
        return;
    }
    
    sqlite3_bind_int(stmt, 1, STATUS_DELIVERED);
    sqlite3_bind_int64(stmt, 2, static_cast<sqlite3_int64>(message_id));
    
    rc = sqlite3_step(stmt);
    if (rc != SQLITE_DONE) {
        spdlog::error("Failed to mark message as delivered: {}", sqlite3_errmsg(db_));
    } else {
...
void Storage::MarkSent(uint64_t message_id) {
    ...
    rc = sqlite3_step(stmt);
    if (rc != SQLITE_DONE) {
        spdlog::error("Failed to mark message as sent: {}", sqlite3_errmsg(db_));
    } else {
...
void Storage::MarkPending(uint64_t message_id) {
    ...
    rc = sqlite3_step(stmt);
    if (rc != SQLITE_DONE) {
        spdlog::error("Failed to mark message as pending: {}", sqlite3_errmsg(db_));
    } else {
        int changes = sqlite3_changes(db_);
...
bool Storage::NeedsAck(uint64_t message_id) {
    ...
    const char* sql = "SELECT flags, status FROM messages WHERE id = ?";
    
    sqlite3_stmt* stmt;
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("Failed to prepare select statement: {}", sqlite3_errmsg(db_));
        return false;
    }
...
void Storage::SaveCorrelation(uint64_t message_id, uint64_t correlation_id, const std::string& original_sender) {
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("Failed to prepare correlation insert: {}", sqlite3_errmsg(db_));
        return;
    }
    
    sqlite3_bind_int64(stmt, 1, static_cast<sqlite3_int64>(message_id));
    sqlite3_bind_int64(stmt, 2, static_cast<sqlite3_int64>(correlation_id));
    sqlite3_bind_text(stmt, 3, original_sender.c_str(), -1, SQLITE_TRANSIENT);
    
    rc = sqlite3_step(stmt);
    if (rc != SQLITE_DONE) {
        spdlog::error("Failed to save correlation: {}", sqlite3_errmsg(db_));
    } else {
...
std::string Storage::FindOriginalSenderByCorrelation(uint64_t correlation_id) {
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("Failed to prepare find sender statement: {}", sqlite3_errmsg(db_));
        return "";
    }
    
    sqlite3_bind_int64(stmt, 1, static_cast<sqlite3_int64>(correlation_id));
...
uint64_t Storage::FindMessageIdByCorrelation(uint64_t correlation_id) {
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        message_id = static_cast<uint64_t>(sqlite3_column_int64(stmt, 0));
...
uint64_t Storage::FindMessageIdByCorrelationAndDestination(uint64_t correlation_id, const std::string& destination) {
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("Failed to prepare find message_id by destination: {}", sqlite3_errmsg(db_));
        return 0;
    }
...
void Storage::MarkAckReceived(uint64_t message_id, const std::string& ack_sender) {
    ...
    if (rc != SQLITE_OK) {
        spdlog::warn("Failed to prepare select original_sender: {}", sqlite3_errmsg(db_));
        return;
    }
    
    sqlite3_bind_int64(select_stmt, 1, static_cast<sqlite3_int64>(message_id));
    ...
    if (original_sender == ack_sender) {
        ...
        if (rc == SQLITE_OK) {
            sqlite3_bind_int64(del_stmt, 1, static_cast<sqlite3_int64>(message_id));
...
std::vector<PendingMessage> Storage::LoadExpiredSent(int timeout_seconds) {
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("Failed to prepare select expired sent: {}", sqlite3_errmsg(db_));
        return {};
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        pending.id = static_cast<uint64_t>(sqlite3_column_int64(stmt, 0));
        
        Message msg;
        msg.SetType(static_cast<MessageType>(sqlite3_column_int(stmt, 1)));
        msg.SetFlags(static_cast<uint8_t>(sqlite3_column_int(stmt, 2)));
        msg.SetCorrelationId(static_cast<uint64_t>(sqlite3_column_int64(stmt, 3)));
...
std::vector<PendingMessage> Storage::LoadPendingMessagesOnly(const std::string& client_name) {
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("Failed to prepare select pending messages: {}", sqlite3_errmsg(db_));
        return {};
    }
...
std::vector<PendingMessage> Storage::LoadPendingRepliesForSenderOnly(const std::string& sender_name) {
    ...
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        spdlog::error("Failed to prepare select pending replies: {}", sqlite3_errmsg(db_));
        return {};
    }
