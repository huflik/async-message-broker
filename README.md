...
1. ZeroMQ сокеты работают в неблокирующем режиме
2. В основном потоке используется zmq_poll с небольшим таймаутом
3. После каждого опроса все готовые дескрипторы обрабатываются, а управление передаётся io_context::poll() или io_context::run_for()
