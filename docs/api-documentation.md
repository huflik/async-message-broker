...
| Метод | Описание |
| :--- | :--- |
| bool SendMessage(Message msg) | Отправляет сообщение клиенту (принимает по значению, поддерживает перемещение) |
...
#### Определение интерфейса
    ...
    public:
        ...
        virtual std::shared_ptr<Session> UpsertClient(const std::string& name, std::shared_ptr<Session> new_session) = 0;
...
| virtual std::shared_ptr<Session> UpsertClient(const std::string& name, std::shared_ptr<Session> new_session) | Выполняет атомарную операцию регистрации или замены сессии клиента |
