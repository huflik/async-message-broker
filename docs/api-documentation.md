...
| void SendToClient(zmq::message_t identity, zmq::message_t data, std::function<void(bool)> callback) | Отправляет сообщение клиенту |
...
### Router (Интерфейс ISessionManager)
Управление сессиями клиентов и маршрутизация сообщений.
...
### ScopedMetricsTimer
RAII-таймер для автоматического измерения времени выполнения.
...
| :--- | :--- | :--- |
| broker_messages_received_total | Counter | Получено сообщений |
| broker_messages_sent_total | Counter | Отправлено сообщений |
| broker_messages_failed_total | Counter | Ошибок доставки |
| broker_acks_received_total | Counter | Получено ACK |
| broker_messages_expired_total | Counter | Просрочено сообщений |
| broker_offline_delivered_total | Counter | Доставлено офлайн |
| broker_clients_registered_total | Counter | Регистраций клиентов |
| broker_clients_unregistered_total | Counter | Отключений клиентов |
| broker_clients_timeout_total | Counter | Отключений по таймауту |
| broker_active_sessions | Gauge | Активных сессий |
| broker_pending_send_queue_size | Gauge | Размер очереди отправки |
| broker_message_processing_duration_seconds | Histogram | Время обработки |
| broker_payload_size_bytes | Histogram | Размер сообщений |
